...
- [x] 执行 `testDebugUnitTest`（29 个 JVM 测试）、`assembleDebug` 与 `lintDebug`。

## 首页滚动热路径优化（2026-08-21）

- [x] 首页恢复 Compose 平台默认 fling，移除额外 1.12 倍速度的自定义衰减，避免快滑时的过冲与边缘取消抖动。
- [x] 滚动期间缓存筛选、模板映射和排序结果；卡片边界改为非 UI 状态的拖拽命中缓存，避免每次布局更新触发额外重组。
- [x] 移除每张卡进入可视区时的 480ms 入场动画，并将背景雾层改为直接渐变合成，避免全屏模糊离屏渲染。
- [x] 正常浏览时不再为每张卡保留布局编辑的 `animateItem` 与变换图层；外框环境阴影保持为唯一静态阴影，长按编辑时才恢复弹性落位、上浮和旋转。
- [x] 精确时刻事件的首页倒计时与卡片“整天”展示精度对齐，统一按分钟刷新，避免多张卡每秒同时触发无可见数值变化的重组。
- [x] 将 Hero 环境过渡由多层全屏渐变/Quiet Zone 叠绘合成为单次直接渐变绘制，保留环境色路径并降低快速滑动时的全屏 GPU 合成负担。
- [ ] 仍需在中低端真机用 Perfetto/JankStats 验证冷启动后首页拖动、快速 fling、长按重排和大字体下的实际帧率。
...
- 新增 `AdaptiveTransitionLayer`：用直接合成的环境渐变和复杂背景 Quiet Zone 消除图片与卡片区域的硬边界，避免滚动时全屏模糊离屏渲染。
- 新增 `AdaptiveCardSurface`：作为现有卡片渲染器的半透明纸张外框，保留所有既有模板、卡片内容和 Small/Wide 两列/跨列规则。
- 背景进入使用约 850ms 淡入；卡片在滚动进入视区时直接呈现，避免逐项入场动画拖慢快速浏览；顶层模板硬阴影已移除，改由环境色阴影承接。
